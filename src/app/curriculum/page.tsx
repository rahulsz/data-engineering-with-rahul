...
import { Compass, Database, GitBranch, BarChart3, Rocket, ChevronRight, BookOpen } from "lucide-react";
    ...
    <div className="max-w-4xl mx-auto py-12 px-6">
      <div className="mb-12">
        <div className="flex items-center gap-2 mb-3">
          ...
          <span className="font-mono text-xs tracking-widest text-brand-500 uppercase">Learning Path</span>
        </div>
        ...
        {CURRICULUM.map((phase, idx) => {
          ...
          return (
            <RevealContent key={phase.phase} delay={idx * 0.1}>
              <div className="bg-white dark:bg-zinc-900/50 border border-zinc-200 dark:border-zinc-800 rounded-2xl overflow-hidden shadow-sm hover:shadow-md transition-shadow">
                <div className="p-6">
                  <div className="flex items-center gap-3 mb-6">
                    <div className="w-10 h-10 rounded-xl flex items-center justify-center" style={{ backgroundColor: phase.color === 'violet' ? '#8b5cf620' : phase.color === 'blue' ? '#3b82f620' : phase.color === 'emerald' ? '#10b98120' : phase.color === 'amber' ? '#f59e0b20' : phase.color === 'rose' ? '#f43f5e20' : '#6366f120' }}>
                      <Icon className="w-5 h-5" style={{ color: phase.color === 'violet' ? '#8b5cf6' : phase.color === 'blue' ? '#3b82f6' : phase.color === 'emerald' ? '#10b981' : phase.color === 'amber' ? '#f59e0b' : phase.color === 'rose' ? '#f43f5e' : '#6366f1' }} />
                    </div>
                  ...
                  <div className="mt-8">
                    {phase.weeks.find(w => w.status === 'available') ? (
                      <Link 
                        ...
                        className="inline-flex items-center gap-2 font-semibold text-brand-600 dark:text-brand-400 hover:text-brand-700 dark:hover:text-brand-300 transition-colors cursor-pointer group"
                      >
                        Start Phase
